...
A final gate review found that GOG authentication advanced the durable lifecycle before checking whether its captured local credential generation was stale. Primary credential replacement now holds the credential lifecycle lock while it validates the expected local generation, advances and publishes the durable lifecycle, writes the replacement file, and increments the local generation. A stale response returns before any durable advance; deterministic logout and overlapping-login races verify that it neither recreates credentials nor invalidates the newer account's ledger.

The same review found that resolver reevaluation could treat a copy's one-reference canonical as independent exact-match evidence. Reevaluation now excludes that self-only canonical from candidate discovery, while a second copy attached to the canonical still counts as corroboration. If no independent candidate wins, the resolver reuses the standalone canonical ID, refreshes its source metadata, and clears any unsupported Steam identity instead of allocating another fallback. Reset, old-version, independent-candidate, and sibling-corroboration tests cover the boundary.

A subsequent gate pass found that same-version automatic decisions returned before candidate discovery, so a copy first projected while Steam was unavailable could never converge after Steam metadata appeared. Non-user automatic decisions now reevaluate on every projection. Accepted current targets remain eligible evidence; self-only unmatched/review fallbacks remain excluded. Equivalent decisions preserve their canonical ID, timestamps, and allocation count, while newly available or changed evidence receives a fresh decision timestamp. Focused and 1,500-copy scale tests verify convergence without no-op churn.

When reevaluation moves the sole relationship from a standalone canonical to an independent target, the projection transaction now merges non-conflicting facets, detail snapshots, and preferences into the target before deleting the zero-reference source canonical. Existing target preferences and same-locale/country snapshots win. A canonical with any remaining relationship is preserved with its dependents. Room-backed tests verify both cleanup and shared-canonical retention so obsolete rows cannot create later title ambiguity.

The cross-check found that schema 26 had two branch-installation shapes: the first published schema omitted ownership ledgers, while a later schema-26 rewrite added them before version 27. The immutable `26.json` history is restored to the first published export. Migration 26→27 accepts both shapes by creating the version-26 ledger tables when absent before adding `lifecycle_generation`; a later-shape ledger and its presence rows are retained. Offline migration tests execute both paths, and instrumentation coverage compiles both equivalent cases without using the occupied emulator.

Steam readiness uses the same private lifecycle state without another Room migration. A per-source ready generation is cleared atomically whenever the lifecycle advances and is persisted only after the matching Steam license transaction commits. Steam account identity changes are written synchronously inside the lifecycle transition; a mutex serializes those transitions, license replacement, and logout cleanup so a captured old callback cannot write or mark a newer generation ready. Snapshot and resolution paths require exact current-generation readiness before reading license-backed rows. The marker survives process restart, while a crash between the Room commit and readiness persistence fails closed until Steam supplies another license list.
