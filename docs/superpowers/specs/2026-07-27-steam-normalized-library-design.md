...
**Status:** Approved design; staged implementation in progress (Stages 0–1 complete)
...
- Analytics may record bounded public titles, AppIDs, storefront IDs, routes, URLs, review/discussion content IDs, and fixed feature/source/match/outcome categories when analytics is enabled. It must not include passwords, API keys, signing secrets, authentication headers/cookies/tokens, SteamIDs, usernames/profiles, personal/account or entitlement associations, install paths, or private user-entered search text.
...
- Diagnostic attributes are enum-allowlisted and sanitized again at the storage boundary. Typed public attributes may contain titles, AppIDs, storefront IDs, routes, validated URLs, cursors, and content IDs alongside event categories, counts, durations, source names, capability names, reason codes, error classes, HTTP status, and short hashed correlations.
- Diagnostic events and reports exclude passwords, API keys, signing secrets, authentication material, account/profile identity, ownership/entitlement associations, local paths, private search text, credential-bearing URLs, and sensitive query parameters. Public game/catalog/community data is not redacted merely because GameNative uses it. Existing raw logcat and crash-log exports remain visibly separate actions.
