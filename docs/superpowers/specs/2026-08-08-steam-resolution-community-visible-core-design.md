...
| R12 | Genuine Epic exclusives have no Steam identity and cannot persist source-native canonical detail metadata | Integrated and host-verified | Closed | Complete-Steam-unmatched-only Epic CMS fallback persists exact `CanonicalGameMetadata` with `EPIC_CMS` provenance and source-aware media policy; owning GameNative regressions pass, and `tools/steam-resolver-poc/reports/epic-fallback-validation.json` records the live Alan Wake 2 response without a fabricated AppID |
