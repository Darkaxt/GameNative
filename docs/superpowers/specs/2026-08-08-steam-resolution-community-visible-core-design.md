...
|---|---|---|
| Steam resolution + Fix match | `31` / `1.1.3-rc5` through corrected key-gate RC `34` / `1.1.3-rc8`; trailer/cache/card-action/support-prompt correction `36` / `1.1.3-rc10` (`35` / RC9 was withheld) | `v1.1.3-rc5` through `v1.1.3-rc10`; only published releases count as acceptance candidates |
...
| R11 | Signed RC10 can show separate Steam and Epic copies of the same public game; the Epic copy's detail remains unavailable while the Steam copy's detail works | Confirmed signed-device blocker investigation | Deliverable 1 resolver acceptance | Identify whether accepted identity, canonical deduplication, or metadata handoff failed; add the owning regression and verify the non-Steam copy receives Steam details without changing Epic action authority on the signed tablet |
...
| U3 | Detail media can consume the available height without keeping the active image/video and carousel navigation visible at the same time | Named work | Stage 4 | In constrained signed-tablet detail layouts, active media and carousel/navigation remain simultaneously visible without clipping or unnecessary scrolling; focused Compose tests cover constrained height plus touch/gamepad navigation |
| U4 | The support prompt can recur on every fresh app process when the user has not permanently tipped | Corrected and signed-live accepted in RC10 | Closed | Timestamp persisted before display; prompt appeared once after upgrade and remained absent across two cold relaunches, with seven-day boundary and clock-rollback tests |
...
| D5 | Current Steam movie payloads use direct HLS/DASH fields, so trailers were omitted when only legacy WebM/MP4 objects were parsed | Corrected and signed-live accepted in RC10 | Closed | Current-HLS and legacy parser tests pass; source-revision invalidation refreshed old caches; DREDGE showed two moving trailers as media 1/18 and 2/18 before screenshots on SM-X910 |
| D6 | Trailer autoplay is permanently silent because `SteamVideoHero` fixes ExoPlayer volume at zero and exposes no mute/unmute action | Confirmed signed-device omission | Stage 4 | Keep autoplay muted by default, add an explicit accessible touch/gamepad mute toggle, cover lifecycle/media transitions, and verify audible signed-tablet playback without unexpectedly changing system volume |
