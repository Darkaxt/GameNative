...
| U4 | The support prompt can recur on every fresh app process when the user has not permanently tipped | Implement now | Deliverable 1 next corrected RC | Persist prompt-show time before display; suppress repeat prompts for seven days with boundary/clock-rollback tests and signed relaunch acceptance |
