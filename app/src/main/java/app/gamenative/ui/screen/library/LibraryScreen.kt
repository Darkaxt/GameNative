...
internal fun LibraryScreenContent(
    ...
    val recTeaserVisible = state.cards.firstOrNull()?.isRecTeaser == true
    ...
    fun openCard(card: LibraryCard) {
        when (val identity = card.identity) {
            ...
            is LibraryCardIdentity.Canonical -> {
                supersedeRouteRequests()
                val canonical = canonicalCard(identity.key)
                if (canonical == null) {
                    SnackbarManager.show(context.getString(R.string.canonical_copy_state_changed))
                    onRefresh()
                    return
    ...
    fun focusedLibraryCard(): LibraryCard? {
        ...
        return state.cards.getOrNull(focusedIndex)?.takeUnless { it.isRecommended }
    }

    fun toggleFocusedFavorite(): Boolean {
        val card = focusedLibraryCard() ?: return false
        ...
        if (selectedCardIdentity == null) {
            ...
            Box(modifier = Modifier.fillMaxSize()) {
                ...
                val showFavoritesEmptyState = state.currentTab == LibraryTab.FAVORITES &&
                    favoritesLoaded && !state.isLoading && state.cards.isEmpty()
                if (showEmptyStateSplash) {
                    ...
                    if (currentPaneType == PaneType.CAROUSEL) {
                        LibraryCarouselPane(
                            ...
                            onNavigate = { card ->
                                if (card.isRecTeaser) {
                                    if (!card.isRecLoading && !recDisclosureShown) {
                                        onRecTeaserTapped()
                                    }
                                } else {
                                    openCard(card)
            ...
            val selectedCanonicalIdentity = selectedCardIdentity as? LibraryCardIdentity.Canonical
            val selectedCanonicalCard = selectedCanonicalIdentity?.key?.let(canonicalCard)
            val presentationCard = selectedPresentationCard
            if (
                showCanonicalDetail &&
                selectedCanonicalIdentity != null &&
        ...
        if (copiesSheetCardKey == null && selectedCardIdentity == null && !state.isOptionsPanelOpen && !isSystemMenuOpen && !isSteamMatchPickerOpen) {
            val libraryActions = if (state.isSearching) {
                ...
                listOf(
                    ...
                    focusedLibraryCard()?.let { card ->
                        GamepadAction(
                            button = GamepadButton.X,
                            labelResId = if (card.favoriteAppIds.any(favorites::contains)) {
