...
internal fun toggleFavorite(context: Context, appId: String, gameName: String?) {
    toggleFavorite(context, setOf(appId), gameName)
}

internal fun toggleFavorite(context: Context, appIds: Set<String>, gameName: String?) {
    val favorite = FavoritesManager.toggle(appIds) ?: return
    val message = when {
